...
* at least some familiarity with the usage of `ggplot2` (such as what was presented during the [2013](https://github.com/berkeley-scf/r-bootcamp-2013/) or [2014 UC Berkeley SCF/D-Lab R Bootcamp](https://github.com/berkeley-scf/r-bootcamp-2014/)),
...
* The code you use to call ggplot and create a graphic is fairly intuitive.  This is because of the "grammar of graphics" that it adheres to, and we'll get back to that in a bit.
...
## Tidy data ^`r Citep(bib, "Wickham_2014", .opts = list(cite.style = "numeric"))` (`reshape2` $\rightarrow$ `tidyr`)
...
## Aside: the philosophy of Edward Tufte
...
## Starting easy: a scatterplot
```{r simple-scatterplot1, fig.cap=" ", message=FALSE, eval=FALSE}
library(ggplot2)
...
```{r simple-scatterplot2, fig.cap=" ", fig.height=9, fig.width=11.5, dpi=300, fig.align='center', message=FALSE, echo=FALSE, cache=FALSE}
library(ggplot2)
ggplot(data=ncaa.basketball[["games"]], aes(x=lscore, y=wscore)) + 
	geom_point() + 
	xlab("Points scored by losing team") +
...
```{r statbinhex1, fig.cap=" ", eval=FALSE}
ggplot(data=ncaa.basketball[["games"]], aes(x=lscore, y=wscore)) + 
...
```{r statbinhex2, fig.cap=" ", fig.height=9, fig.width=10.5, dpi=300, fig.align='center', message=FALSE, echo=FALSE, cache=FALSE}
ggplot(data=ncaa.basketball[["games"]], aes(x=lscore, y=wscore)) + 
...
## Improving the scatterplot
```{r geomhex1, fig.cap=" ", message=FALSE, eval=FALSE}
ggplot(data=ncaa.basketball[["games"]], aes(x=lscore, y=wscore)) + 
	# default is bins=c(30,30) or just 30 since no. of bins for x and y are same.
	# here alternatively specifying binwidth:
...
```{r geomhex2, fig.cap=" ", fig.height=9, fig.width=10.5, dpi=300, fig.align='center', message=FALSE, echo=FALSE, cache=FALSE}
ggplot(data=ncaa.basketball[["games"]], aes(x=lscore, y=wscore)) + 
	# default is bins=c(30,30) or just 30 since no. of bins for x and y are same.
	# here alternatively specifying binwidth:
	geom_hex(stat = "binhex", binwidth=c(4,4)) + 
...
`geom_contour()`   | `stat_contour()`   | 10 `pretty` breakpoints that cover the [range of z]
...
## Other `ggplot` transformations (excluding identity)
...
```{r summarizebball, fig.cap=" ", message=FALSE, cache=FALSE}
library(dplyr)    # new and way faster than plyr
...
## Where to go for help with `ggplot`
...
## Other "companion" packages to `ggplot2`
...
```{r googlevis1, results='asis', message=FALSE, eval=FALSE}
library(googleVis)
...
```{r googlevis2, results='asis', message=FALSE, echo=FALSE, cache=FALSE}
library(googleVis)
# data.frame with >=4 cols: x, y, id, time. color and size optional
mc = gvisMotionChart(ncaa.basketball[["team.season.summ"]], 
	idvar="team.name", timevar="season", 
	xvar="win.pct", yvar="tourney.seed",
	options=list(width=750, height=650,
		hAxis="{title:'Percent of regular season games won'}",
        vAxis="{title:'Tournament seed'}"))
# plot(mc)
print(mc, 'chart')
```
...
- rMaps
- gridSVG
- [plotly](http://ropensci.org/blog/2014/04/17/plotly/)
- [networkD3](http://christophergandrud.github.io/networkD3/)
- gg2v
- ggvis
- rggobi, ggobi, DescribeDisplay
...
*** =pnotes
* rMaps: one nice usage case is pkg rCrimemap
gridSVG
...
+ the [notes](https://github.com/freekh/deck.js-notes) extension, 
